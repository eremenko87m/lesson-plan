...
def main():
    wb,mode=get_workbook()
    # The sync fingerprint includes BOTH the Yandex workbook and this parser.
    # This means a parser bugfix forces one fresh rebuild even when the
    # workbook itself has not changed.
    workbook_fp=hashlib.sha256(wb).hexdigest()
    parser_fp=hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
    fp=hashlib.sha256((workbook_fp+parser_fp).encode('utf-8')).hexdigest()
    if DATA.exists():
        try:
            old=json.loads(DATA.read_text('utf-8'))
            if old.get('sourceFingerprint')==fp:
                print('Workbook and parser unchanged — nothing to update.');return
        except:pass
    ...
    payload={
        ...
        'sourceMode':mode,
        'sourceSheet':'PLAN',
        'workbookFingerprint':workbook_fp,
        'parserFingerprint':parser_fp,
